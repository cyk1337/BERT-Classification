...
flags.DEFINE_string("task_name", "domainclf", "The name of the task to train.")

flags.DEFINE_string("vocab_file", "chinese_L-12_H-768_A-12/vocab.txt",
                    "The vocabulary file that the BERT model was trained on.")

flags.DEFINE_string(
    "output_dir", "tmp/domain_output/",
...
flags.DEFINE_bool(
    "do_single_predict", False,
    "Whether to run the model in inference mode on single instance.")

flags.DEFINE_string(
    "X_input", "",
    "Single sentence for prediction")
...
def main(_):
    ...
    if not FLAGS.do_train and not FLAGS.do_eval and not FLAGS.do_predict and not FLAGS.do_single_predict:
        raise ValueError(
            "At least one of `do_train`, `do_eval`, `do_predict' or `do_single_predict` must be True.")
    ...
    if FLAGS.do_predict:
        ...
        output_label_file = os.path.join(FLAGS.output_dir, "test_predictions.tsv")
        # chaiyekun added
    ...
    #
    #     features = []
    #     for i, pred_example in enumerate(predict_examples):
    #         feature = convert_single_example(i, pred_example, label_list, FLAGS.max_seq_length, tokenizer)
    #
    ...
    #
    #     predict_drop_remainder = True if FLAGS.use_tpu else False
    #     predict_input_fn = input_fn_builder(features, FLAGS.max_seq_length, False, predict_drop_remainder)
    #     result = estimator.predict(input_fn=predict_input_fn)
    #
    #     tf.logging.info("***** Predict results ... *****")
    #
    #     y_true, y_pred = [], []
    #     for (i, prediction) in enumerate(result):
    #         y_true.append(predict_examples[i].label)
    #         probabilities = prediction["probabilities"]
    #         assert len(probabilities) == len(label_list)
    #         # labels
    #         id_ = np.argmax(np.asarray(probabilities))
    #         label = label_list[id_]
    #         y_pred.append(label)
    #
    #     print("corr cnt:", sum([1 for i in range(len(y_pred)) if y_pred[i] == y_true[i]]))
    #     tf.logging.info("***** Generating report ... *****")
